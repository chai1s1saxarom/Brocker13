...
- Архитектурная схема.
- Модель данных и форматы API.
- Выбор протокола: REST/HTTP (простая интеграция и быстрый MVP).
- План реализации модулей.

### Этап 2. Базовая функциональность
- Реализация pub/sub и queue.
- Подписки и множественные consumers.
- Доставка сообщений, ack, retries.
- Подключение persistent storage.
- SDK и тестовые сервисы.

## Что можно улучшить дальше

- Exactly-once semantics через idempotency key + dedup store.
- Prometheus-совместимый формат метрик.
- AuthN/AuthZ.
- Кластеризация и репликация.
- Поддержка протоколов MQTT/AMQP.
